...
public class PatientRepository {
    ...
    public boolean checkUidExist(String uid){
        boolean res = false;
        try {
            res = new CheckTagExistAsyncTask(patientDao)
                    .execute(uid).get();
        }catch (Exception e) {
            e.printStackTrace();
        }
    ...
    // ***
    // check if tag exist Async
    private static class CheckTagExistAsyncTask extends AsyncTask<String, Void, Boolean> {
        private PatientDao patientDao;
        private Boolean res;
        private CheckTagExistAsyncTask(PatientDao patientDao) {
            this.patientDao = patientDao;
        }
        @Override
        protected Boolean doInBackground(String... strings) {
            if(!patientDao.checkUidExist(strings[0]).equals("0")) {
                res = true;
            }
            else {
                res = false;
