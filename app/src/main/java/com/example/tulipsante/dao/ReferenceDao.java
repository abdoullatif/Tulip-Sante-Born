...
public interface ReferenceDao {
    ...
    @Query("SELECT * from patient " +
            ...
            "where reference.idMedecin1 like :idMedecin and patient.prenomPatient like :searchData and medecin.prenomMedecin like :searchData")
    LiveData<List<PatientXRefXDoc>> patientReferredByMe(String idMedecin, String searchData);

    @Query("SELECT * from patient " +
            "inner join consultation on consultation.idPatient = patient.idPatient " +
            "inner join reference on reference.idConsultation = consultation.idConsultation " +
            "inner join medecin on medecin.idMedecin = reference.idMedecin1 " +
            "where reference.idMedecin2 like :idMedecin and patient.prenomPatient like :searchData and medecin.prenomMedecin like :searchData")
    LiveData<List<PatientXRefXDoc>> patientReferredByOthers(String idMedecin, String searchData);
