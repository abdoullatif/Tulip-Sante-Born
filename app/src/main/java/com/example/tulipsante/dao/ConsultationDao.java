...
//    List<ConsultationUMedecin> getConsultationRecordSortOldest(String idPatient, String index);
    ...
    @Query("SELECT *,nomPatient,prenomPatient,genrePatient, dateNaissancePatient, groupeSanguinPatient, photoPatient from consultation inner join patient on consultation.idPatient LIKE patient.idPatient AND consultation.idMedecin LIKE :idMedecin ORDER BY dateConsultation Desc limit :index,15")
    LiveData<List<HistoriqueTacheMedecin>> getHistoriqueTache(String idMedecin, String index);

    @Query("SELECT *,nomPatient,prenomPatient,genrePatient, dateNaissancePatient, groupeSanguinPatient, photoPatient from consultation inner join patient on consultation.idPatient LIKE patient.idPatient AND consultation.idMedecin LIKE :idMedecin AND consultation.dateConsultation LIKE :dateHistorique ORDER BY dateConsultation ASC")
